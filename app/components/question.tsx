...
export function QuestionBox({props, questionNumber, next, saved}:any) {
    ...
    const checkAnswer = async (event: any) => {
        ...
        if(props.japanese === event.target[0].value || props.kanji === event.target[0].value || props.romanji?.toLowerCase() === event.target[0].value.toLowerCase()) {
            setScore(1);
    ...
    return(
        <div className="bg-amber-200 px-12 py-6 rounded-md shadow-md">
            ...
            <p className="text-lg">Japanese/Romanji:</p>
            {!answerBoxView &&
